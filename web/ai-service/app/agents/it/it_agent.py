"""IT Agent — retrieval + generation for IT documentation."""
...
_log = get_logger()
_SYSTEM = (Path(__file__).parent.parent.parent / "prompts" / "it_system.md").read_text(encoding="utf-8")


def _build_messages(context: str, question: str, history: list[dict]) -> list[dict]:
    ctx = context.strip() or "(No relevant IT documents were found.)"
    user_content = f"### Context\n\n{ctx}\n\n### Question\n\n{question}"
    return [{"role": "system", "content": _SYSTEM}] + history[-10:] + [{"role": "user", "content": user_content}]


async def it_agent_node(state: GraphState, retrieval_tool: RetrievalTool, llm_service: LLMService) -> dict:
    ...
    try:
        ...
        _log.warning("it_agent_retrieval_failed", error=str(exc))
        return {
            "answer": "I couldn't find that information in the uploaded IT documentation.",
            "retrieved_context": "", "citations": [],
            "errors": [*state.get("errors", []), f"IT retrieval error: {exc}"],
    ...
    try:
        ...
        _log.warning("it_agent_llm_failed", error=str(exc))
        return {
            "answer": "I encountered an error generating an IT response.",
            "retrieved_context": retrieval.context, "citations": citations_to_dicts(retrieval.citations),
            "errors": [*state.get("errors", []), f"IT LLM error: {exc}"],
            "provider": "", "model": "", "prompt_tokens": 0, "completion_tokens": 0,
            "latency_ms": round((time.monotonic() - t0) * 1000, 1),
        }

    latency = round((time.monotonic() - t0) * 1000, 1)
    _log.info("it_agent_complete", tokens=llm_resp.usage.total_tokens, latency_ms=latency)
    return {
        ...
        "latency_ms": latency,
        "metadata": {**state.get("metadata", {}), "it_chunks_found": retrieval.chunks_found},
    }
